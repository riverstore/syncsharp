...
namespace SyncSharp.Business
{
	...
	public class Reconciler
	{
		...
		public Reconciler(FileList srcList, FileList tgtList, SyncTask task, String id)
		{
			...
			_summary.logFile = @".\Profiles\" + id + @"\" + task.Name + ".log";
		}
				...
				else if (_tgtDirtyFilesList.containsPriKey(relativePath))
				{
					...
					else if (srcFlag.Equals("C") && _srcRenameList.containsPriKey(relativePath))
					{
						...
						if (_tgtDirtyFilesList.containsPriKey(relativePath))
						{
							...
							if (_tgtCleanFilesList.containsPriKey(delFile))
							{ _tgtCleanFilesList.removeByPrimary(delFile); }
							if (File.Exists(_tgtPath + delFile))
								File.Delete(_tgtPath + delFile);
						}
					}
					else
					{
