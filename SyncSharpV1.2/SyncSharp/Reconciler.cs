            ...
            foreach (var myRecord in srcList.Primary)
            {
                ...
                if (Directory.Exists(srcFile))
                {
                    if (!Directory.Exists(tgtFile))
                    {
                        ...
                        Logger.WriteLog((int)Logger.LogType.Copy, null, 0, null, 0, tgtFile, 0, null, 0, null);
                    }
                }
                else
                {
            ...
            foreach (var myRecord in srcList.Primary)
            {
                ...
                if (Directory.Exists(tgtFile))
                {
                    if (!Directory.Exists(srcFile))
                    {
                        ...
                        Logger.WriteLog((int)Logger.LogType.Copy, srcFile, 0, null, 0, null, 0, null, 0, null);
                    }
                }
                else
                {
                ...
                case cFlag:
                    {
                        ...
                        if (!_srcRenameList.containsPriKey(relativePath))
                        {
                            preview.tgtFlag = dFlag; // added in deleted flag for first time sync
        ...
        private void PreviewFoldersCleanup()
        {
            foreach (var mySrcRecord in _srcDirtyFoldersList.PriSub)
            {
                ...
                if (srcFlag.Equals(cFlag))
                {
                    ...
                    preview.srcFlag = cFlag;
                    preview.tgtFlag = dFlag; //change to deleted instead of clean
            ...
            foreach (var myTgtRecord in _tgtDirtyFoldersList.PriSub)
            {
                ...
                if (tgtFlag.Equals(cFlag))
                {
                    ...
                    preview.srcFlag = dFlag; //change to deleted instead of clean
                    preview.tgtFlag = cFlag;
                }
                ...
                case cFlag:
                    {
                        ...
                        else
                        {
                            ...
                            long fileSize = new FileInfo(_tgtPath + delRelativePath).Length;
                ...
                case cFlag:
                    {
                        ...
                        else
                        {
                            ...
                            if (_srcCleanFilesList.containsPriKey(delRelativePath))
                            {
                                ...
                                long fileSize = new FileInfo(_srcPath + delRelativePath).Length;
        ...
        private void SyncFoldersCleanup()
        {
            foreach (var mySrcRecord in _srcDirtyFoldersList.PriSub)
            {
                ...
                if (srcFlag.Equals(cFlag))
                {
                    ...
                    Logger.WriteLog((int)Logger.LogType.Copy, _srcPath + relativePath, 0, null, 0, null, 0, null, 0, null);
                    if (!Directory.Exists(_srcPath + relativePath))
            ...
            foreach (var myTgtRecord in _tgtDirtyFoldersList.PriSub)
            {
                ...
                if (tgtFlag.Equals(cFlag))
                {
                    ...
                    Logger.WriteLog((int)Logger.LogType.Copy, null, 0, null, 0, _tgtPath + relativePath, 0, null, 0, null);
                    if (!Directory.Exists(_srcPath + relativePath))
                    { Directory.CreateDirectory(_srcPath + relativePath); }
                    if (!Directory.Exists(_tgtPath + relativePath))
                    { Directory.CreateDirectory(_tgtPath + relativePath); }
