...
namespace SyncSharp.Business
{
	public class Detector
	{
		...
		private CustomDictionary<string, string, FileUnit> backupFiles;
		...
		public Detector(String metaDataDir, SyncTask syncTask)
		{
			...
			backupFiles = new CustomDictionary<string, string, FileUnit>();
		...
		private void compareSrcDirs(FileUnit u, String folderRelativePath)
		{
			if (metaDataExists())
			{
				...
				else
				{
					...
					backupFiles.add(folderRelativePath, u.Hash, u);
				}
			}
			else
			{
			...
			if (metaDataExists())
			{
				if (sMetaData.Primary.ContainsKey(relativePath))
				{
					...
					else
					{
						...
						backupFiles.add(relativePath, u.Hash, u);
						sMetaData.removeByPrimary(relativePath);
					}
				}
				else
			...
			if (metaDataExists())
			{
				foreach (var item in sMetaData.Primary)
				{
					if (item.Value.IsDirectory)
					{
						...
						//backupFiles.add(item.Key, item.Value.Hash, item.Value);
					}
					else
					{
