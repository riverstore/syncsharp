...
namespace SyncSharp.Business
{
	public class Detector
	{
		...
		{
			int sRevPathLen = (_task.Source.EndsWith("\\")) ? _task.Source.Length - 1 : _task.Source.Length;
			int tRevPathLen = (_task.Target.EndsWith("\\")) ? _task.Target.Length - 1 : _task.Target.Length;
		...
		private void RemoveExclusions(int sRevPathLen, int tRevPathLen)
		{
			...
			foreach (FileUnit u in _tgtFiles)
			{
				...
				else
				{
					...
					//if (!_task.Filters.IsFileExcluded(new FileInfo(u.AbsolutePath)) && !_task.Filters.IsTargetDirExcluded(u.AbsolutePath) &&
					//  !_fileExclusions.Contains(relativePath))
					//{

					if (!_fileExclusions.Contains(relativePath))
						tempDestFiles.Add(u);
					//  }
					//  else
					//  {
					//    if (_sMetaData.containsPriKey(relativePath))
					//      _sMetaData.removeByPrimary(relativePath);
					//    if (_tMetaData.containsPriKey(relativePath))
					//      _tMetaData.removeByPrimary(relativePath);
					//    _fileExclusions.Add(relativePath);
					//    u.Hash = Utility.computeMyHash(u);
		...
		private void GetCurrentSrcInfo(List<FileUnit> srcFiles, Stack<string> stack)
		{
			while (stack.Count > 0)
			{
				...
				{
					//if (!_task.Filters.IsSourceDirExcluded(folderName))
					//{
					...
					//else
					//{
					//  _sMetaData.removeByPrimary(folderName);
					//}
				}
			}
		}
		...
		private void GetCurrentTgtInfo(List<FileUnit> destFiles, Stack<string> stack)
		{
			while (stack.Count > 0)
			{
				...
				foreach (string folderName in Directory.GetDirectories(folder))
				{
					//if (!_task.Filters.IsTargetDirExcluded(folderName))
					//{
					stack.Push(folderName);
					...
					//else
					//{
					//  _tMetaData.removeByPrimary(folderName);
					//}
				}
			}
		}
			...
			if (MetaDataExists())
			{
				foreach (var item in _sMetaData.Primary)
				{
					if (item.Value.IsDirectory)
					{
						...
						//_backupFiles.add(item.Key, item.Value.Hash, item.Value);
					}
					else
					{
